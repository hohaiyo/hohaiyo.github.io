$(function(){
    ...
    //实名认证的禁止点击按钮
        ...
        var btn = $('#mian-form-submit');
        
        $(Idnum).hover(function(){
            var Val = $('#input-idnum').val();
            var Idcard = $('#input-idcard').val();
            if(Val.length == 18 && Idcard.length !== ''){
                $(btn).addClass('pointer').removeClass('not-allowed').removeAttr('disabled');
            }else{
                $(btn).removeClass('pointer').addClass('not-allowed');
            } 
        },function(){
            var Val = $('#input-idnum').val();
            var Idcard = $('#input-idcard').val();
